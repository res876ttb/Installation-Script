...
# position of gmp library
gmpLibrary=$HOME/.pkg/gmp-6.1.2/lib
...
echo Building $pkg...
cd $src
cm "./configure --prefix=$td --with-gmp-lib=$gmpLibrary" "configure $pkg"
cm "make -j" "build $src"
cm "make check" "check $src"
