...
required="
  ...
  $HOME/.script/env-mpich-3.2.1.sh \
"
...
echo Building $pkg...
cd $src
cm "env CFLAGS=$cflag CXXFLAGS=$cflag FFLAGS=$cflag FCFLAGS=$cflag ./configure --enable-fortran --enable-cxx --enable-unsupported --enable-optimization=high --prefix=$td" "configure $pkg"
