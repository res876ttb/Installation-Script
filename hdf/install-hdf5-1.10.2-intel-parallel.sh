...
# target directory
td=$HOME/.pkg/hdf5-1.10.2-intel

# script path
sp=$HOME/.script/env-$pkg-intel.sh
...
echo Building $pkg...
cd $src
cm "./configure --enable-parallel --enable-fortran --enable-cxx --enable-unsupported --enable-optimization=high --prefix=$td" "configure $pkg"
