...
# target directory
td=$HOME/.pkg/hdf5-1.10.2-pgi

# script path
sp=$HOME/.script/env-$pkg-pgi.sh
...
# ramdisk path
ramdisk=/tmp/ramdisk-$pkg-pgi

# scripts of required packages
required="
  $HOME/.script/env-cmake-3.11.2.sh \
  $HOME/.script/env-zlib-1.2.11.sh \
  $HOME/.script/env-openmpi-3.1.0-pgi.sh \
  $HOME/.script/env-pgi-18.4.sh \
"
...
echo Building $pkg...
cd $src
cm "env CC=pgcc CXX=pgc++ FC=pgf90 CFLAGS=$cflag CXXFLAGS=$cflag FFLAGS=$cflag FCFLAGS=$cflag ./configure --enable-fortran --enable-cxx --enable-unsupported --enable-optimization=high --prefix=$td" "configure $pkg"
